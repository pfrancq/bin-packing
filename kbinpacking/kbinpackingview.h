...
/** The KBinPackingView class provides the view widget for the document instance connected to it and is displayed
 * as a MDI child window in the main view area of the KBinPackingApp class instance. The KBinPackingApp class also has an eventFilter()
 * method that gets installed on every KBinPackingView instance to control events of the type QEvent::Close.	
 * The document connected to the view instance keeps a list of all view that represent the document contents as there
 * can be more than one view. Views get created in KBinPackingApp::createClient() and automatically added to the list of views.
 * The KBinPackingView class inherits QWidget as a base. Another possible inheritance besides specialized widgets could be
 ...
 * event filter can only manage a forward implementation. If the QCloseEvent is received by the KBinPackingView, the overwritten
...
class KBinPackingView : public QWidget
{
  ...
  public:
    ...
    KBinPackingDoc *getDocument() const;
    /** gets called to redraw the document contents if it has been modified */
		void update(KBinPackingView* pSender);
    /** contains the implementation for printing functionality and gets called by KBinPackingApp::slotFilePrint() */
    void print(QPrinter *pPrinter);
		
  protected:
    /** overwritten QWidget::closeEvent() to catch closing views. Does nothing, as the closeEvents for
    * KBinPackingView's are processed by KBinPackingApp::eventFilter(), so this overwitten closeEvent is necessary
